...
from magma.adapters import (
    Adapter,
    ParallelAdapter,
    AdapterWrapper,
    ParallelAdapterWrapper,
)
from magma.image_prefix import ImagePrefix
from magma.sampling import generate
from magma.utils import build_labels, is_url, print_main, download_checkpoint
from magma.image_input import ImageInput
from magma.transforms import get_transforms

from atman_magma.gpt_neo.modeling_gpt_neo import GPTNeoForCausalLM
from atman_magma.config  import get_gptj_config
from .gpt_neo.steered_sampling import generate_with_atman

# ------------------------- Magma main class ----------------------------------


class Magma(nn.Module):
    def __init__(self, config = None, device=None, fake_magma_num_layers = None):
        super().__init__()

        if config is None:
            config = get_gptj_config()

        self.device = device or torch.device(
            "cuda" if torch.cuda.is_available() else "cpu"
        )
        if fake_magma_num_layers is not None:
            print('WARNING: you are loading a fake Magma model, whose parameters are not valid. Use this for profiling/benchmarking only and not for evaluations.')
            config.num_layers = fake_magma_num_layers

        self.config = config

        self.lm = GPTNeoForCausalLM(config = config).to(self.device)
    ...
    def generate(
        ...
        ## making sure there's no manipulation
        self.lm.suppression_factors = None
        self.lm.suppression_token_indices = None
    ...
    def generate_steered(
        ...
        suppression_token_indices = None
    ):
        """
        Generates captions for a batch of embeddings.
        """
        assert len(suppression_factors) == embeddings.shape[0], f'Expected len(suppression_factors): {len(suppression_factors)} to be equal to embeddings batch size: {embeddings.shape[0]}'
        assert len(suppression_token_indices) == embeddings.shape[0], f'Expected len(suppression_factors): {len(suppression_token_indices)} to be equal to embeddings batch size: {embeddings.shape[0]}'

        self.lm.suppression_factors = suppression_factors
        self.lm.suppression_token_indices = suppression_token_indices

        output = generate_with_atman(
            self,
            embeddings=embeddings,
            max_steps=max_steps,
            temperature=temperature,
...
#        ), f"in training, captions should be padded to sequence length ({self.seq_len}), but are length {captions.shape[1]}"
    ...
    @classmethod
    def from_checkpoint(cls, checkpoint_path, config = None, device = 'cpu', fake_magma_num_layers=None):
        """
        ...
        model = cls(config = config, fake_magma_num_layers = fake_magma_num_layers)
