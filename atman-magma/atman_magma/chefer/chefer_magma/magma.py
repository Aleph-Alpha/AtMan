...
from magma.utils import build_labels, print_main, download_checkpoint
from magma.image_input import ImageInput
from magma.transforms import get_transforms

from ...config  import get_gptj_config

# ------------------------- Magma main class ----------------------------------


class CheferMagma(nn.Module):
    def __init__(self, config = None, device=None, fake_magma_num_layers = None):
        ...
        self.config = config
        self.lm = get_gptj(config = self.config).to(self.device)
        ...
        ## should require grad by default
        for name, param in self.lm.named_parameters():  # freeze lm weights
            #print(name)
            param.requires_grad = True
