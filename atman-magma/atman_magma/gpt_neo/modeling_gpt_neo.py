...
## stuff for attention manipulation
from ..manipulate_attention import manipulate_attention_scores
...
class GPTNeoAttentionMixin:
    ...
    def _attn(self, query, key, value, causal_mask, masked_bias, attn_dropout, attention_mask=None, head_mask=None, scale_attn=None, full_bf16=False, modified_causal_attention_mask = None, manipulate_attn_scores_after_scaling=False):
        if not full_bf16 and query.dtype is torch.bfloat16:
            attn_weights = torch.matmul(query.float(), key.transpose(-1, -2).float())
        else:
            attn_weights = torch.matmul(query, key.transpose(-1, -2))

        ## modify attn_weights before scaling
        if modified_causal_attention_mask is not None and manipulate_attn_scores_after_scaling is False:

            attn_weights = manipulate_attention_scores(
                attention_scores=attn_weights,
                attention_mask=causal_mask,
                modified_causal_attention_mask=modified_causal_attention_mask,
                apply_softmax=False
            )
        ...
        ## modify attn_weights after scaling
        if modified_causal_attention_mask is not None and manipulate_attn_scores_after_scaling is True:
        ...
        ## set values below -10_000 to -10_000
        attn_weights[attn_weights < -10000.] = -10000.
...
class GPTNeoSelfAttention(nn.Module, GPTNeoAttentionMixin):
    ...
    def forward(
        ...
        modified_causal_attention_mask= None,
        ...
        ## WARNING: I would ignore the indexing for now, will just use self.bias as a reference to generate modified attn masks
        ...
        attn_output, attn_weights = self._attn(
            ...
            modified_causal_attention_mask=modified_causal_attention_mask,
            manipulate_attn_scores_after_scaling=manipulate_attn_scores_after_scaling
        )
...
class GPTNeoAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        try:
            ...
            print("You're probably trying to load a FakeMagma model, will replace layer_id > 1 with -1")
            self.attention_type = self.attention_layers[-1]
...
class GPTNeoModel(GPTNeoPreTrainedModel):
    def __init__(self, config):
        ...
        for i in range(config.num_layers):
            print(f'Preparing layer idx: {i} of {config.num_layers - 1}')
            h_layers.append(
                to_gpu(
                    GPTNeoBlock(
                        config,
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            layer_modified_causal_attention_mask = modified_causal_attention_mask
            if manipulate_layers is not None and len(manipulate_layers) > 0 and manipulate_layers != [None]:
                if i not in manipulate_layers:
                    layer_modified_causal_attention_mask = None
                    #print(f"setting manipulation None {i}")
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                outputs = block(
                    ...
                    use_cache=use_cache,
                    output_attentions=output_attentions,
                    modified_causal_attention_mask=layer_modified_causal_attention_mask,
                    manipulate_attn_scores_after_scaling=manipulate_attn_scores_after_scaling
                )
...
class GPTNeoForCausalLM(GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        if self.suppression_token_indices is not None and self.suppression_factors is not None:

            if input_ids is not None:
                if past_key_values is not None:
                    ## past_key_values  is a list of list of tensors each of shape: [1, 16, seq, 256]
                    '''
                    source: https://huggingface.co/docs/transformers/main_classes/output#transformers.modeling_outputs.BaseModelOutputWithPast.past_key_values
                    past_key_values (tuple(tuple(torch.FloatTensor)), optional, returned when use_cache=True
                    is passed or when config.use_cache=True) — Tuple of tuple(torch.FloatTensor) of length
                    config.n_layers, with each tuple having 2 tensors of shape (batch_size, num_heads, sequence_length, embed_size_per_head))
                    and optionally if config.is_encoder_decoder=True 2 additional tensors of shape
                    (batch_size, num_heads, encoder_sequence_length, embed_size_per_head).
                    '''
                    seq_len = past_key_values[-1][0].shape[2]
                else:
                    seq_len = input_ids.shape[1]
            else:
                seq_len = inputs_embeds.shape[1]

            modified_causal_attention_mask = self.get_modified_causal_attention_mask(
                seq_len = seq_len,
                suppression_token_indices = self.suppression_token_indices,
                suppression_factors = self.suppression_factors
            )
        else:
            modified_causal_attention_mask = None
        ...
        transformer_outputs = self.transformer(
            ...
            embs=embs,
            modified_causal_attention_mask=modified_causal_attention_mask,
            manipulate_attn_scores_after_scaling=self.manipulate_attn_scores_after_scaling,
            manipulate_layers=self.layers
        )
